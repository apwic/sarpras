...
import {
    closeModalSU,
    openModalSU,
    getAllUnsignedStaffAction,
    setStaffToRoleStart,
} from '../../superuser/action';
...
class SuperUserModal extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.superUserModalOpen !== this.props.superUserModalOpen) {
            this.setState({
                show: this.props.superUserModalOpen,
                searchQuery: '',
                allUnsignedStaff: this.props.allUnsignedStaff,
                selectedId: null,
            });
        }
    ...
    handleAddStafftoRole = () => {
        if (this.state.selectedId !== null) {
            this.props.addStafftoRoleFunction(
                this.state.selectedId.toString(),
    ...
    handleSearchQueryChange = (event) => {
        ...
        const filteredData = this.props.allUnsignedStaff.filter(
            (staff) =>
    ...
    unsignedStaffList = (data) => {
        return data.map((staff) => {
            ...
            const rowClassName = `row__staff__modal${
                isSelected ? ' selected' : ''
            }`;
            return (
                <div
            ...
            <div>
                ...
                >
                    ...
                    <Modal.Body className="staff-list border-top">
                        {this.state.allUnsignedStaff.length > 0
                            ? this.unsignedStaffList(
                                  this.state.allUnsignedStaff,
                              )
                            : this.noUnsignedStaff()}
                    </Modal.Body>
                    <Modal.Footer>
                        <Button
                            variant="secondary"
                        ...
                        <Button
                            ...
                            variant="primary"
                            onClick={() => this.handleAddStafftoRole()}
                            disabled={
                                this.state.selectedId === null ? true : false
                            }
                        >
                            Tambahkan
                        </Button>
...
const mapDispatchToProps = (dispatch) => {
    return {
        openModalFunction: (selectedRole) =>
            dispatch(openModalSU(selectedRole)),
        closeModalFunction: () => dispatch(closeModalSU()),
        getAllUnsignedStaffFunction: () =>
            dispatch(getAllUnsignedStaffAction()),
        addStafftoRoleFunction: (staffId, roleId) =>
            dispatch(setStaffToRoleStart(staffId, roleId)),
    };
};
