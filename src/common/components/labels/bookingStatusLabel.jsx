...
class BookingStatusLabel extends React.Component {
    ...
    componentDidMount() {
        if (this.props.status === bookingStatusConstant.PENDING) {
            ...
            this.setState({color: "#D70D0D"});
        } else if (this.props.status === bookingStatusConstant.ON_VERIFICATION) {
            this.setState({color: "#EEE600"});
        } else if (this.props.status === bookingStatusConstant.WAITING_FOR_PAYMENT) {
            this.setState({color: "#EEE600"});
        } else if (this.props.status === bookingStatusConstant.PAYMENT_SUCCESS) {
            this.setState({color: "#00B140"});
        } else if (this.props.status === bookingStatusConstant.ENDED) {
            this.setState({color: "#00B140"});
        } else if (this.props.status === bookingStatusConstant.WAITING_FOR_RATING) {
            this.setState({color: "#00B140"});
    ...
    render() {
        return (
            <div className="status-label" style={{backgroundColor: this.state.color}}>
